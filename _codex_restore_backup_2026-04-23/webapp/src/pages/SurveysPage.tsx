...
type SurveysSection = 'builder' | 'clients' | 'employees' | 'ratings';
...
const SurveysPage: React.FC<SurveysPageProps> = ({ apiClient }) => {
  ...
  const [activeSection, setActiveSection] = useState<SurveysSection>('builder');
  ...
  return (
    <div className="page surveys-page surveys-page--app-sidebar">
      <header className="surveys-hero">
        ...
        <button type="button" className="surveys-button surveys-button--primary" onClick={() => void data.load()} disabled={data.isLoading}>
          Обновить
        </button>
      </header>
      ...
      {activeSection === 'employees' ? (
        <EmployeeSurveyAnalytics analytics={employeeAnalytics} isLoading={employeeLoading} error={employeeError} onRefresh={loadEmployeeAnalytics} />
      ) : null}
...
const EmployeeSurveyAnalytics: React.FC<{
  ...
  error: string | null;
  onRefresh: () => void;
}> = ({ analytics, isLoading, error, onRefresh }) => {
  const lineOption = useMemo(
    () => createLineOption(
      analytics?.monthlyScores.map((item) => formatMonth(item.month)) ?? [],
      analytics?.monthlyScores.map((item) => item.averageOverallScore) ?? [],
      'Средний балл',
      '#0f766e',
    ),
    [analytics],
  ...
  return (
    <AnalyticsPanel
      eyebrow="Опросы сотрудников"
      title="Как сотрудники оценивают клиентов"
      description="Внутренняя оценка клиента после обращения: качество данных, коммуникация и повторы."
      actionLabel="Обновить"
      onAction={onRefresh}
      isLoading={isLoading}
    >
      ...
      <KpiGrid
        items={[
          { label: 'Заполненные анкеты', value: numberFormatter.format(analytics?.totalAssessments ?? 0), hint: 'сотрудник -> клиент' },
          { label: 'Средний балл', value: formatScore(analytics?.averageOverallScore), hint: 'из 5' },
          { label: 'Индекс коммуникации', value: analytics?.averageInteractionQualityIndex === null || analytics?.averageInteractionQualityIndex === undefined ? '-' : `${Math.round(analytics.averageInteractionQualityIndex)}%`, hint: 'по оценкам' },
          { label: 'Повторные обращения', value: formatPercent(analytics?.repeatedRequestShare), hint: 'по клиентам' },
        ]}
      />
      {!analytics || analytics.totalAssessments === 0 ? (
        ...
        <>
          <div className="surveys-dashboard-grid">
            <Card title="Динамика">
              {analytics.monthlyScores.length === 0 ? <EmptyState text="Недостаточно данных." /> : <EChartsWrapper option={lineOption} style={{ height: 220 }} />}
            </Card>
            <Card title="Характер обращений">
              <EChartsWrapper option={createBarOption(analytics.requestRepeatStatuses)} style={{ height: 220 }} />
            </Card>
          </div>
          <div className="surveys-dashboard-grid surveys-dashboard-grid--three">
          ...
          <Card title="Рейтинг клиентов">
            <DataTable
              headers={['Клиент', 'Средний балл', 'Индекс', 'Обращения', 'Повторы']}
              rows={analytics.clientRatings.slice(0, 20).map((item) => [
                `${item.clientName}${item.clientBin ? ` · ${item.clientBin}` : ''}`,
                formatScore(item.averageOverallScore),
                `${Math.round(item.averageInteractionQualityIndex)}%`,
                numberFormatter.format(item.taskCount),
