...
Функция ВыполнитьPOST(ОтносительныйПуть, Заголовки, ТелоСтрока) Экспорт
    ...
    ВставитьЗаголовок(Заголовки, "Content-Length", СтрДлина(ТелоСтрока));
...
Функция ОтправитьСообщениеMobileBot(ExternalChatId, Текст, БИН,
    ...
    Ответ = ВыполнитьPOST("/api/integrations/1c/messages", Заголовки, ТелоJSON);
...
Функция ПолучитьИсториюMobileBot(ExternalChatId, ChatId=Неопределено, DialogId=Неопределено) Экспорт
    ...
    Путь = "/api/integrations/1c/messages?" + QS;
...
Функция ПроверитьСоединениеMobileBot() Экспорт
    Попытка
        ...
        Ответ = ВыполнитьGET("/api/health", Заголовки);
