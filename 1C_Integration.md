...
&НаСервере
Функция ЗагрузитьФайлНаСервере(ПолноеИмяФайла, ДанныеФайла)
    Возврат РеквизитФормыВЗначение("Объект").ЗагрузитьМедиаMobileBot(ПолноеИмяФайла, ДанныеФайла);
КонецФункции
...
Функция СформироватьHTMLВложений(Вложения)
    ...
    Для Каждого Вложение Из Вложения Цикл
        ...
        Если ТипВложения = "image" Тогда
            ...
            Результат = Результат + СформироватьHTMLКартинкиДляIE(СсылкаПредпросмотра, ИмяФайла, Размеры.Ширина, Размеры.Высота, Base64Content, MimeType);
...
Функция СформироватьHTMLОжидающихВложений()
    ...
    Для Каждого Вложение Из Вложения Цикл
        ...
        Результат = Результат +
            ...
            "<a href='#' class='pending-upload-remove' data-button-id='RemoveUpload_" + MediaId + "' onclick='return false;'>Г—</a>" +
            "</div>";
    КонецЦикла;
...
// Рекомендуется предварительно сжимать изображения (например, в JPEG 80%) на стороне 1С 
// для уменьшения объема передаваемого трафика и ускорения загрузки.
