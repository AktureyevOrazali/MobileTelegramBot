    ...
    *,
    ...
    with _lock:
        ...
        if operator_request_rows:
            # Используем уже созданные переменные automation_clause и operator_filter_clause
            response_query_automation_clause = (
                f"AND TRIM(LOWER(m.author)) NOT IN ({automation_author_placeholders})"
                if automation_author_placeholders
                else ""
            )
            response_query_operator_clause = (
                f"AND TRIM(LOWER(m.author)) IN ({', '.join('%s' for _ in operator_names)})"
                if operator_names
                else ""
            )
            ...
            for request_row in operator_request_rows:
                ...
                query_parts = [
                    "SELECT m.created_at, m.author, m.dialog_id",
                    "FROM messages m",
                    "LEFT JOIN chat_dialogs cd ON cd.id = m.dialog_id",
                    "LEFT JOIN chats c ON c.chat_id = m.chat_id",
                    "WHERE m.direction = 'outgoing'",
                ...
                if dialog_id is not None:
                    ...
                    params.append(dialog_id)
                if response_query_automation_clause:
                    query_parts.append(response_query_automation_clause)
                    params.extend([name.lower() for name in AUTOMATION_AUTHOR_NAMES])
                if response_query_operator_clause and operator_names:
                    operator_response_placeholders = ", ".join("%s" for _ in operator_names)
                    query_parts.append(f"AND TRIM(LOWER(m.author)) IN ({operator_response_placeholders})")
                    params.extend([name.lower() for name in operator_names])
                if assigned_bins is not None:
                    query_parts.append(
                        f"  AND COALESCE(cd.bin, c.bin) IN ({placeholders})"
                    )
